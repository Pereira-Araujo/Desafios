...
const SinglePage = () => {
  const [callCost, setCallCost, origin,
    setOrigin, destination, setDestination,
    faleMaisPlan, setFaleMaisPlan,
    minutes, setMinutes, isVisible] = useContext(GlobalStateContext);
    ...
    <Container>
      ...
      <Box>
        <FormComponent
          ...
          Options={areaCode.map((area) => { return (<MenuItem key={area} value={area}>{area}</MenuItem>) })} />
        ...
        <FormComponent
          ...
          Options={plans.map((type) => { return (<MenuItem key={type} value={type}>{type}</MenuItem>) })}
        />
      </Box>
      ...
      {isVisible !== true ?
        ...
        : (<BoxPlanComponent
          ...
          layBy={difference(totalNoPlan(callCost, minutes), totalWithPlan(callCost, minutes, Number(faleMaisPlan)))}
        />
        )}
